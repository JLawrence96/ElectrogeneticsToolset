function [hImage, hText, hXText] = heatmap2(mat, xlab, ylab, textmat, varargin)
% HEATMAP displays a matrix as a heatmap2 image
%
% USAGE:
% [hImage, hText, hTick] = heatmap2(matrix, xlabels, ylabels, textmatrix, 'param', value, ...)
%
% INPUTS:
...
%   heatmap2. The value of the parameter is a LineStyle specification, for example,
...
% heatmap2(data, cellstr(('A':'J')'), mean(data,2), '%0.0f%%',...
%         'Colormap', 'money', 'Colorbar', true, 'GridLines', ':',...
%         'TextColor', 'b')
% For detailed examples, see the associated document heatmap2_examples.m
...
% Store heatmap2 properties in axes for callbacks
axesInfo = struct('Type', 'heatmap2', 'Parameters', p, 'FontScaleFactor', ...
...
%     warning('heatmap2:graphics2figurecolormap', 'The UseFigureColormap false option with colorbar is not supported in versions R2014b and above. In most such cases UseFigureColormap false is unnecessary');
% end
    

end

% Visualize heatmap2 image
...
if ~strcmp(p.GridLines,'none')
    ...
    for i = 1:diff(xlim)-1
        line('Parent',p.hAxes,'XData',[i i]+.5, 'YData', ylim, 'LineStyle', p.GridLines,'LineWidth',3);
    end
    for i = 1:diff(ylim)-1
        line('Parent',p.hAxes,'XData',xlim, 'YData', [i i]+.5, 'LineStyle', p.GridLines,'LineWidth',3);
    end
end
end
...
if isempty(p.axesInfo) % Not previously a heatmap2 axes
...
% Extract heatmap2 parameters for callback
function axInfo = getHeatmapAxesInfo(axH)
axInfo = get(axH, 'UserData');
try
    if ~strcmp(axInfo.Type, 'heatmap2')
        axInfo = [];
    end
catch %#ok<CTCH>
    axInfo = [];
...
% Function to make the heatmap2 have the green, white and red effect
n = min(data(:));
x = max(data(:));
if x == n, x = n+1; end
